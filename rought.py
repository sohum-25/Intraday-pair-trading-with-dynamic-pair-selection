...
from datetime import datetime
from TradingFunction import MockCalculateParameters

def fetch_ltp_for_symbol(database_path, fetch_table, symbol):
    try:
...
def fetch_latest_ltp(symbols, database_path, insert_table, fetch_table):
    ...
    for symbol in symbols:
        result = fetch_ltp_for_symbol(database_path, fetch_table, symbol)
        latest_ltp_values[result[0]] = result[1]
...
if __name__ == '__main__':
    ...
    Fund_Response=fyers.funds()
    MasterCapital= 0.95 * Fund_Response['fund_limit'][8]['equityAmount']
    
    TotalCapital = int(MasterCapital)
    inTrade = False
    activeLongPosition = False
    activeShortPosition = False

    # Generate 10 sets of variables with different names
    for i in range(2):
        # Create variable names with numbers at the end
        total_capital_var = f'TotalCapital_{i}'
        in_trade_var = f'inTrade_{i}'
        active_long_var = f'activeLongPosition_{i}'
        active_short_var = f'activeShortPosition_{i}'

        # Create variables with the given initial state
        locals()[total_capital_var] = TotalCapital
        locals()[in_trade_var] = inTrade
        locals()[active_long_var] = activeLongPosition
        locals()[active_short_var] = activeShortPosition
   ...
   # Specify the file path
    ...
    while True:
        ...
        and (current_hour < 15 or (current_hour == 15 and current_minute <= 17))
        )
        ):
            fetch_latest_ltp(symbols, database_path, insert_table, fetch_table)
            TotalCapital_0,inTrade_0,activeLongPosition_0,activeShortPosition_0 = MockCalculateParameters(database_path, symbol_list, 0, TotalCapital_0,fyers,inTrade_0,activeLongPosition_0,activeShortPosition_0)
    ...
    'slNo']]
    conn = sqlite3.connect(database_path)
    df.to_sql('Trades', conn, index=False, if_exists='append')
    conn.close()